...
log_cf2dns = Logger('cf2dns.log', level='info')
...
def report(text):
    ...
    if isinstance(CHAT_ID, list):
        for ID in CHAT_ID:
            http.request('GET',URL+"?chat_id="+ID+"&text="+text)
    else:
        http.request('GET',URL+"?chat_id="+CHAT_ID+"&text="+text)

def get_optimization_ip(key, type):
    try:
        http = urllib3.PoolManager()
        headers = headers = {'Content-Type': 'application/json'}
        api = http.request('GET','https://api.hostmonit.com/get_license?license=' + key,headers=headers)
        query = json.loads(api.data.decode('utf-8'))
        if query["code"] != 200:
            report("cf2dns警告:\n查询可用次数失败！\ncode:" + str(text["code"]) + "\ninfo:" + text['info'])
        else:
            if query["count"] <= COUNT:
                report("cf2dns警告:\n可用次数少于设定值" + str(COUNT) + "次！\n当前可用次数:" + str(query["count"]))
        ...
        if text["code"] != 200:
            report("cf2dns警告:\n获取ip失败！\ncode:" + str(text["code"]) + "\ninfo:" + text['info'])
            if text["info"] == 'The number of times available has reached the limit!':
                return get_optimization_ip("o1zrmHAF", type)
        return text
    except Exception as e:
        log_cf2dns.logger.error(e)
        return None
    ...
    try:
        ...
        if create_num == 0:
            for info in s_info:
                ...
                if TYPE == 'v4,v6':
                    ret = cloud.change_record(domain, info["recordId"], sub_domain, cf_ip, recordType2, line, TTL)
                    if(DNS_SERVER != 1 or ret["code"] == 0):
                        log_cf2dns.logger.info("CHANGE DNS SUCCESS: ----Time: " + str(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())) + "----DOMAIN: " + domain + "----SUBDOMAIN: " + sub_domain + "----RECORDLINE: "+line+"----RECORDID: " + str(info["recordId"]) + "----VALUE: " + cf_ip )
                    else:
                        log_cf2dns.logger.error("CHANGE DNS ERROR: ----Time: " + str(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())) + "----DOMAIN: " + domain + "----SUBDOMAIN: " + sub_domain + "----RECORDLINE: "+line+"----RECORDID: " + str(info["recordId"]) + "----VALUE: " + cf_ip + "----MESSAGE: " + ret["message"] )
            ...
            for i in range(create_num):
                ...
                if(DNS_SERVER != 1 or ret["code"] == 0):
                    ...
                    log_cf2dns.logger.error("CREATE DNS ERROR: ----Time: " + str(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())) + "----DOMAIN: " + domain + "----SUBDOMAIN: " + sub_domain + "----RECORDLINE: "+line+"----RECORDID: " + str(info["recordId"]) + "----VALUE: " + cf_ip + "----MESSAGE: " + ret2["message"] )
                if TYPE == 'v4,v6':
                    ret = cloud.create_record(domain, sub_domain, cf_ip, recordType2, line, TTL)
                    if(DNS_SERVER != 1 or ret["code"] == 0):
                        log_cf2dns.logger.info("CREATE DNS SUCCESS: ----Time: " + str(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())) + "----DOMAIN: " + domain + "----SUBDOMAIN: " + sub_domain + "----RECORDLINE: "+line+"----VALUE: " + cf_ip )
                    else:
                        log_cf2dns.logger.error("CREATE DNS ERROR: ----Time: " + str(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())) + "----DOMAIN: " + domain + "----SUBDOMAIN: " + sub_domain + "----RECORDLINE: "+line+"----RECORDID: " + str(info["recordId"]) + "----VALUE: " + cf_ip + "----MESSAGE: " + ret2["message"] )
...
def main(cloud):
    ...
    if len(DOMAINS) > 0:
        try:
            cfips = get_optimization_ip(KEY, TYPE)
            ...
            if TYPE == 'v4,v6':
                cfips = get_optimization_ip(KEY, "v6")
                if cfips == None or cfips["code"] != 200:
                    log_cf2dns.logger.error("GET CLOUDFLARE IP ERROR: ----Time: " + str(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())) + "----MESSAGE: " + str(cfips["info"]))
                    return
                cf_cmips = cfips["info"]["CM"]
                                ...
                                for record in ret["data"]["records"]:
                                    if record["line"] == "移动" or record["line"] == "联通" or record["line"] == "电信":
                                        ...
                                        if(retMsg["code"] == 0):
                                            ...
                                            log_cf2dns.logger.error("DELETE DNS ERROR: ----Time: "  + str(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())) + "----DOMAIN: " + domain + "----SUBDOMAIN: " + sub_domain + "----RECORDLINE: "+record["line"] + "----MESSAGE: " + retMsg["message"] )
                        ret = cloud.get_record(domain, 100, sub_domain, recordType2)
                        if DNS_SERVER != 1 or ret["code"] == 0 :
                            if DNS_SERVER == 1 and "Free" in ret["data"]["domain"]["grade"] and AFFECT_NUM > 2:
                                AFFECT_NUM = 2
                            cm_info = []
