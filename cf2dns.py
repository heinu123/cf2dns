...
# 发送指定的 chat_id 如果需要发送给多个 chat_id 请使用list表
CHAT_ID = ''
...
#v4为筛选出IPv4的IP  v6为筛选出IPv6的IP 默认双栈
TYPE = ['v4','v6']
...
def report(text):
    if isinstance(CHAT_ID, list):
        for ID in CHAT_ID:
            requests.get(URL+"?chat_id="+ID+"&text="+text)
    else:
...
def get_optimization_ip(key, type):
    try:
        ...
        response = requests.get('https://api.hostmonit.com/get_license?license=' + key,headers=headers)
